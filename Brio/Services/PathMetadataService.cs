...
public class PathMetadataService
{
    ...
    private void Load(PathTarget target, ref byte[] data)
    {
        ...
        {
            Brio.Log.Warning($"Path metadata store '{target}' is invalid or outdated; using defaults.");
            if(target == PathTarget.User)
                TryBackupInvalidUserStore();
        }
        catch(Exception ex)
        {
    ...
    private void TryBackupInvalidUserStore()
    {
        try
        {
            if(!File.Exists(_userFile))
            ...
            var backup = _userFile + ".invalid.bak";
            File.Copy(_userFile, backup, overwrite: true);
            File.Delete(_userFile);
        }
        catch(Exception ex)
        {
            Brio.Log.Warning(ex, "Could not backup invalid user path metadata file");
        }
    }
