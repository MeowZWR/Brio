...
public class PosingGraphicalWindow : Window, IDisposable
{
    ...
    private void DrawGraphics(PosingCapability posing, ActorAppearanceCapability appearance)
    {
        ...
        using(var child = ImRaii.Child("###body_pane", new Vector2(contentWidth, -1), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
            if(child.Success)
            {
                if(_selectedPane == 0)
                {
                    DrawBoneSection("body", true, posing, BoneCategory.身体页面);
                }
            }
        }
        ...
        if(_selectedPane == 0)
        {
            ...
            using(var child = ImRaii.Child("###armor_pane", new Vector2(contentWidth, -1), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
            {
                if(child.Success)
                {
                    DrawBoneSection("armor", true, posing, BoneCategory.身体页面);
                }
            }
            ...
            using(var child = ImRaii.Child("###face_pane", new Vector2(contentArea.X - (35 * ImGuiHelpers.GlobalScale), -1), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
            {
                if(child.Success)
                {
                    switch(currentAppearance.Customize.Race)
                    {
                        ...
                        case Races.Lalafel:
                        case Races.AuRa:
                            DrawBoneSection("human_head", true, posing, BoneCategory.身体页面);
                            break;

                        case Races.Miqote:
                            DrawBoneSection("miqote_head", true, posing, BoneCategory.身体页面);
                            break;

                        case Races.Hrothgar:
                            DrawBoneSection("hrothgar_head", true, posing, BoneCategory.身体页面);
                            break;
                        ...
                        case Races.Viera:
                            ...
                            {
                                case 1:
                                    DrawBoneSection("viera_head_a", true, posing, BoneCategory.身体页面);
                                    break;
                                case 2:
                                    DrawBoneSection("viera_head_b", true, posing, BoneCategory.身体页面);
                                    break;
                                case 3:
                                    DrawBoneSection("viera_head_c", true, posing, BoneCategory.身体页面);
                                    break;
                                case 4:
                                    DrawBoneSection("viera_head_d", true, posing, BoneCategory.身体页面);
                                    break;
                                default:
                                    DrawBoneSection("viera_head_a", true, posing, BoneCategory.身体页面);
                                    break;
    ...
    private enum BoneCategory { 身体页面, Hand, Minor }
