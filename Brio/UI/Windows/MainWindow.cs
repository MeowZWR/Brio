...
public class MainWindow : Window, IDisposable
{
        ...
        using(var container = ImRaii.Child("###entity_hierarchy_container", new Vector2(-1, containerHeight), true))
        {
            if(container.Success)
            {
                ...
                if(_entityManager.SelectedEntityIds.Count > 1)
                {
                    ...
                    ImGui.Text($"{_entityManager.SelectedEntityIds.Count} 个已选中");
                }
            }
        }
    ...
    private void DrawHeaderButtons()
    {
        ...
        using(ImRaii.Disabled(_gPoseService.IsGPosing == false))
        {
            ...
            if(ImBrio.Button(" 项目", FontAwesomeIcon.FolderOpen, new Vector2(line1Width, 0), centerTest: true))
                ImGui.OpenPopup("DrawProjectPopup");

            ImGui.SameLine();
            if(ImBrio.Button(" 资产库", FontAwesomeIcon.Book, new Vector2(line1Width, 0), centerTest: true))
