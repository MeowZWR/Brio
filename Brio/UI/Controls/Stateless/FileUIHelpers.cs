...
public class FileUIHelpers
{
    ...
    public static void DrawImportPoseMenuPopup(string tag, PosingCapability? capability, bool showImportOptions = true)
    {
        ...
        if(popup.Success)
        {
            ...
            using(ImRaii.PushColor(ImGuiCol.Button, UIConstants.Transparent))
            {
                ...
                {
                    if(ImBrio.Button("导入选项", FontAwesomeIcon.Cog, new(size.X, 25), centerTest: true, tooltip: "导入选项"))
    ...
    {
        UIManager.Instance.FileDialogManager.SaveFileDialog("导出MCDF文件###export_mcdf_window", "月海角色数据文件(*.mcdf){.mcdf}", "mcdf", "{.mcdf}",
                 (success, path) =>
                 {
    ...
    {
        UIManager.Instance.FileDialogManager.SaveFileDialog("导出场景文件###export_scene_window", "Brio场景文件(*.brioscn){.brioscn}", "brioscn", "{.brioscn}",
            (success, path) =>
            {
                if(success)
                {
