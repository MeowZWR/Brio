    ...
    public unsafe static void DrawFreeCam(string id, BrioCameraCapability capability)
    {
        ...
        using(ImRaii.PushId(id))
        {
            using(ImRaii.Disabled(!capability.IsAllowed))
            {
                ...
                if(ImBrio.FontIconButtonRight("reset", FontAwesomeIcon.Undo, 1f, "重置相机", camera.IsOverridden))
                    camera.ResetCamera();
...

                {
                    ...
                    if(ImBrio.DragFloat2V3("平移", ref rotation, -360, 360, "%.3f", true, ImGuiSliderFlags.AlwaysClamp))
