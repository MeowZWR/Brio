    ...
    public unsafe static void DrawFreeCam(string id, BrioCameraCapability capability)
    {
        ...
        using(ImRaii.PushId(id))
        {
            using(ImRaii.Disabled(!capability.IsAllowed))
            {
                ...
                if(ImBrio.ToggelFontIconButton("portraitMode", FontAwesomeIcon.Mobile, new Vector2(0, 0), camera.IsPortraitMode, tooltip: "竖屏模式"))
                {
                ...
                if(ImBrio.FontIconButtonRight("resetRotation", FontAwesomeIcon.Undo, 1f, "重置", pivoRotation != 0))
                {
                    camera.PivotRotation = 0;
                    capability.Snapshot();
                }
                ...
                if(ImBrio.FontIconButtonRight("resetMovementSpeed", FontAwesomeIcon.Undo, 1f, "重置移动速度", moveSpeed != capability._configurationService.Configuration.Interface.DefaultFreeCameraMovementSpeed))
                {
                    camera.FreeCamValues.MovementSpeed = capability._configurationService.Configuration.Interface.DefaultFreeCameraMovementSpeed;
                    capability.Snapshot();
                }
                ...
                if(ImBrio.FontIconButtonRight("resetMouseSensitivity", FontAwesomeIcon.Undo, 1f, "重置鼠标灵敏度", mouseSpeed != capability._configurationService.Configuration.Interface.DefaultFreeCameraMouseSensitivity))
                {
                    camera.FreeCamValues.MouseSensitivity = capability._configurationService.Configuration.Interface.DefaultFreeCameraMouseSensitivity;
                    capability.Snapshot();
                }
        ...
        using(ImRaii.PushId(id))
        {
            using(ImRaii.Disabled(!capability.IsAllowed))
            {
                if(camera is not null)
                {
                    ...
                    if(ImBrio.FontIconButtonRight("resetRotation", FontAwesomeIcon.Undo, 1f, "重置", pivotRotation != 0))
                    {
                        camera.PivotRotation = 0;
                        capability.Snapshot();
                    }
    ...
    public static unsafe void DrawCameraActorSelect(BrioCameraCapability capability)
    {
        ...
        using(ImRaii.Disabled(capability._entityManager.SelectedEntity is not ActorEntity))
            if(ImBrio.FontIconButton("recenter_on_selected", FontAwesomeIcon.Bullseye, "重新居中到所选角色"))
            {
                ...
                {
                    capability.VirtualCamera.SelectedActorName = $"已选择：[ {actor.FriendlyName} ]";
