    ...
    private unsafe static void DrawTransformOffset(PosingCapability posingCapability)
    {
        ...
        if(popup.Success)
        {
            ...
            if(selectedIsBone.HasValue && !posingCapability.Actor.IsProp)
            {
                ...
                if(bone is not null)
                {
                    ImBrio.DragFloat($"##transformSpeed_1", ref bone.BoneAdjustmentOffset, 0.001f, 10, 0.01f, "偏移", 50);
            ...
            {
                ImBrio.DragFloat($"##transformSpeed_1", ref posingCapability.ModelPosing.TransformOffset, 0.001f, 10, 0.01f, "偏移", 50);
