    ...
    private void DrawModelTransformEditorSingle(PosingCapability posingCapability, bool compactMode = false)
    {
        ...
        using(ImRaii.Disabled(posingCapability.ModelPosing.Freeze == true))
        {
            ...
            if(isProp)
            {
                ...
                (sdidChange, sanyActive) = ImBrio.DragFloat3($"###_transformScale_1", ref realTransform.Scale, offset, FontAwesomeIcon.ExpandAlt, "缩放", enableExpanded: compactMode);
        ...
        if(popup.Success)
        {
            ...
            if(selectedIsBone.HasValue && !posingCapability.Actor.IsProp)
            {
                ...
                if(bone is not null)
                {
                    ...
                    ImBrio.DragFloat($"##transformSpeed_1", ref bone.BoneAdjustmentOffset, 0.001f, 10f, 0.01f, "偏移", 50);
                    bool freezeTransforms = bone.Freeze;
                    if(ImGui.Checkbox("冻结变换", ref freezeTransforms))
                    {
            ...
            {
                ImBrio.DragFloat($"##transformSpeed_1", ref posingCapability.ModelPosing.TransformOffset, 0.001f, 10f, 0.01f, "偏移", 50);
