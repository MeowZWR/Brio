...
public class TimelineSequencerEditor(TimelineService timelineService, ConfigurationService configurationService)
{
    ...
    private static bool DrawComponents(ITimelineHost host, List<TrackKeyframe> selected, TrackKeyframe representative)
    {
        ...
        {
            changed |= DrawCameraComponentToggle(selected, representative, "位置", CameraComponents.Position);
            ImGui.SameLine();
            changed |= DrawCameraComponentToggle(selected, representative, "旋转", CameraComponents.Rotation);
            ImGui.SameLine();
            changed |= DrawCameraComponentToggle(selected, representative, "镜头", CameraComponents.Lens);
        }
        else if(representative.Light.HasValue)
        {
            changed |= DrawLightComponentToggle(selected, representative, "位置", LightComponents.Position);
            ImGui.SameLine();
            changed |= DrawLightComponentToggle(selected, representative, "渲染", LightComponents.Rendering);
        }
        else if(representative.WorldObject.HasValue)
        {
            changed |= DrawWorldObjectComponentToggle(selected, representative, "变换", WorldObjectComponents.Transform);

            if(host.CaptureChannels.Any(c => c.Name == "颜色"))
            {
                ImGui.SameLine();
                changed |= DrawWorldObjectComponentToggle(selected, representative, "颜色", WorldObjectComponents.Color);
            }
        }
        else
        {
            changed |= DrawTransformComponentToggle(selected, representative, "位置", TransformComponents.Position);
            ImGui.SameLine();
            changed |= DrawTransformComponentToggle(selected, representative, "旋转", TransformComponents.Rotation);
            ImGui.SameLine();
            changed |= DrawTransformComponentToggle(selected, representative, "缩放", TransformComponents.Scale);
        }
